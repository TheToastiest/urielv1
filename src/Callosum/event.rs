...
pub struct EventEnvelope {
    ...
    pub ts: std::time::SystemTime,     // when published
    pub source: &'static str,          // "parietal", "cortex", "ui", ...
    pub correlation_id: Option<u64>,   // per interaction/turn id
    pub event: CallosumEvent,
}

impl CallosumEvent {
    /// Try to view the dynamic payload as a concrete type.
    ...
    /// Convenience constructor for Dynamic.
...
pub struct ForwardPolicy {
    pub speak_threshold: f32,   // e.g., 0.60
    pub text_threshold: f32,    // e.g., 0.35
    ...
    pub fn decide(&self, evt: &CallosumEvent) -> Option<ForwardAct> {
        match evt {
            CallosumEvent::EvaluatedInput { input, urgency, familiarity:_, understanding, drive_align, tags:_ } => {
                // route selection already happened upstream: ParietalRoute::Respond etc.
                // If you attach the chosen route to this event, branch on it here.
            ...
            CallosumEvent::ResponseGenerated(s) => {
                if self.policy.autospeak_enabled { Some(ForwardAct::Both { text: s.clone(), speech: s.clone() }) }
                else { Some(ForwardAct::Text(s.clone())) }
            }
            _ => None
        }
    }
