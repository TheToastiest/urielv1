...
impl<B: Backend> Embeddings for ModelEmb<B> {
    fn embed(&self, text: &str) -> std::result::Result<Embedding, Box<dyn Error + Send + Sync>> {
        let model = self.model.lock().map_err(|_| {
            Box::<dyn Error + Send + Sync>::from(io::Error::new(io::ErrorKind::Other, "model lock poisoned"))
        })?;
...
impl<B: Backend> Embedder for SmieHippEmbed<B> {
    ...
    fn embed(&self, text: &str) -> anyhow::Result<Vec<f32>> {
        ...
        // apply diagonal weights (if any), then renormalize
        if let Some(w) = self.oep_w.read().unwrap().as_ref() {
...
fn main() -> Result<()> {
    ...
    let (route_T, route_thr) = load_routercopy_calib("data/runs/router_v4_bal/calib.json")
    ...
    loop {
            ...
            && !input_main.trim_start().to_ascii_lowercase().starts_with("from ")
        {
            ...
            if answer.is_empty() {
                ...
                // --- ROUTER-GATE (calibrated) ---
                let mut force_retrieve_only = false;
                let mut force_store = false;
                let mut _defer_asked = false; // underscore avoids unused warning

                if let Some(sys_rc) = router.as_ref() {
                    // 1) encode → embed → pool (match training length 128)
                    let u_tok = rc_encode_ascii95(&input_main, 128);
                    let u_oh  = rc_tokens_to_one_hot::<B>(&[u_tok], RC_VOCAB, &device); // [1,Tu,V]
                    let u_emb = rc_embed_seq::<B>(u_oh, &sys_rc.embed);                 // [1,Tu,384]
                    let u_pool = rc_mean_over_time::<B>(u_emb);                         // [1,384]

                    // 2) logits → temperature → probs
                    let (logits, _arg_logits_opt) = sys_rc.model.route_logits(u_pool);  // [1,4]
                    let probs = burn::tensor::activation::softmax(logits.div_scalar(route_T), 1);

                    // 3) top-1 class & confidence (apply bias)
                    let p: Vec<f32> = probs.to_data().as_slice::<f32>().unwrap().to_vec(); // len=4
                    let mut pv = [0f32; 4];
                    for i in 0..4 { pv[i] = p[i]; }
                    {
                        // read current bias and perturb probs: p[c] *= exp(bias[c]); renorm
                        let b = *router_bias.read().unwrap();
                        apply_bias_to_probs(&mut pv, b);
                    }
                    let mut pred = 0usize;
                    let mut pmax = pv[0];
                    for i in 1..4 { if pv[i] > pmax { pmax = pv[i]; pred = i; } }

                    // remember what the router chose this turn
                    router_pred_class = Some(pred);


                    // classes: 0=Respond, 1=Retrieve, 2=Store, 3=Defer
                    if pmax >= route_thr.promote {
                        match pred {
                            1 => force_retrieve_only = true,        // Retrieval-only
                            2 => force_store = true,                // Persist immediately
                            3 => { /* println!("Could you clarify that a bit?"); */ _defer_asked = true; }
                            _ => {}
                        }
                    } else if pmax <= route_thr.refuse {
                ...
                if is_question(&input_main) {
                    ...
                    if let Some(pos) = pos_opt {
                        ...
                        if let Some((neg, _)) = cands.first() {
                            ...
                            let eq0 = m.run_inference(&q_for_train).to_data().as_slice::<f32>().map_err(|e| anyhow::anyhow!("{e:?}"))?.to_vec();
                            let ep0 = m.run_inference(&pos).to_data().as_slice::<f32>().map_err(|e| anyhow::anyhow!("{e:?}"))?.to_vec();
                            let en0 = m.run_inference(neg).to_data().as_slice::<f32>().map_err(|e| anyhow::anyhow!("{e:?}"))?.to_vec();
                            drop(m);
